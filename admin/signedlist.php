...
try {
    ...
    // 从数据库里读取数据
    $signedlist_sql = "SELECT * FROM zsctf_signed";
    $signedlist_res = $pdo -> prepare($signedlist_sql);
    $signedlist_res -> execute();
    ...
    echo ('
        <br><div class="mdui-typo" style="font-size:28px;font-weight:500;text-align:center">签到详情</div>
    ...
    // 列出数据
    while ($rst = $signedlist_res->fetch(PDO::FETCH_ASSOC)) {
